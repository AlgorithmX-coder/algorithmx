...
- **Server-side reward persistence is LIVE** (PR #48, 2026-06-30): `Progress.xp` max-merged per week (idempotent by construction, never lowers), badges + stickers derived at read time from `completedAt`, cross-device hydration on lesson mount, /cyberhq rank banner + badge wall. Known gaps vs `implementation.md`: client-computed `totalXp` (server clamp is loose), XP values as magic numbers at call sites (no RULES table yet), no award-path tests. Close these when building the next reward mechanic — the event-claim architecture in `implementation.md` is the target shape.
