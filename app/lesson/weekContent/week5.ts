  ...
  bossShowdown: {
    ...
    phases: [
      // P1 · PILE-ON → COUNTER-CARD: the thread grows on the wall.
      {
        kind: "counterCard",
        attack: 0,
        coach: "You're the upstander here. Tap the hero card!",
        situation: "A shy kid posts his very first drawing in the art app. Kids start dropping laughing emojis on it until he deletes it and goes quiet.",
        situationIcon: "🌀",
        cards: [
          { id: "stand", label: "COMMENT SOMETHING KIND TO HIM", icon: "💪", isRight: true, note: "" },
          { id: "joke", label: "Drop one laughing emoji, it's only one", icon: "💬", isRight: false, note: "One emoji is a brick in the pile. He counts every laugh, including yours." },
          { id: "watch", label: "Scroll past, it isn't your drawing", icon: "👀", isRight: false, note: "Scrolling keeps you clean but leaves him standing alone. Standing BY him is the hero move." },
        ],
      },
      ...
      // never touch the tempting reply buttons.
      {
        kind: "deflectSort",
        attack: 1,
        coach: "Mute every mean echo. Feed the fire NOTHING!",
        actLabel: "MUTE IT",
        actIcon: "🤫",
        passLabel: "LET IT DRIFT",
        items: [
          { id: "echo-build", label: "Echo: 'You always ruin the game!'", icon: "💬", act: true, note: "A mean echo made to sting. Mute it to mist, don't answer it." },
          { id: "reply-caps", label: "'Fire back: YOU'RE the worst!'", icon: "⚡", act: false, note: "That's the fire begging for wood. Let it drift, stay calm." },
          { id: "echo-team", label: "Echo: 'Nobody laughs at YOUR jokes!'", icon: "💬", act: true, note: "Another echo. Mute it, it says nothing true about you." },
          { id: "reply-comeback", label: "'Roast them back in a different chat'", icon: "🌀", act: false, note: "A comeback is still wood on the fire. Let it drift past." },
          { id: "echo-quit", label: "Echo: 'Go back to the baby class!'", icon: "💬", act: true, note: "Mute this one too, then breathe. It can't grow." },
          { id: "reply-meaner", label: "'Screenshot it and clap back HARD'", icon: "🔠", act: false, note: "Clapping back only feeds the flames. Heroes let it drift." },
        ],
      },
      // P3 · LONELY CLOUD → ORDER-STRIKE: lift it one calm step at a time.
      {
        kind: "orderStrike",
        attack: 2,
        coach: "Do the calm steps in order, hero!",
        intro: "Nasty messages start landing in your swim-team chat, all aimed at you. Walk the calm path one step at a time.",
        steps: [
    ...
    weakPoints: [
      { question: "A mean thread about a kid has had no new comments for two whole days. What is most likely happening to it?", answers: ["It gets meaner the longer it sits", "It is fading, pile-ons shrink when nobody feeds them", "It will grow again on its own overnight", "Nothing can ever slow it down"], correctIndex: 1, explanation: "Pile-ons live on new joiners. No new voices, no fuel, so it fades. Your silence helps starve it." },
      { question: "Someone posts one mean comment, then goes quiet and waits. If you blast an angry reply, what does the bully get?", answers: ["Exactly the reaction they were fishing for", "Scared off for good", "Too bored to carry on", "A fair fight, so it ends"], correctIndex: 0, explanation: "A quiet bully is waiting for a reaction. Your angry reply is the wood, and the reaction is the whole point." },
      { question: "Your friend is being bullied but begs you: 'Please don't tell anyone, you'll only make it worse!' What is the kind thing to do?", answers: ["Promise to say nothing, it is their secret", "Wait to see if it stops by itself", "Tell a trusted grown-up, that is how it stops", "Only tell if it starts happening to you too"], correctIndex: 2, explanation: "'Don't tell' keeps the hurt hidden and growing. Telling a grown-up is not snitching, it is how your friend gets real help." },
    ],
    finisher: {
