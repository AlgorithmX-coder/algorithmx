...
export const case17War: WarCase = {
  ...
  test: {
    intro: "Last thing before I sign this off, the test. A fresh batch of scenarios, no hints, and you need to get most of them right. Everything about telling real from fake, put it to work. Ready?",
    ...
    questions: [
      ...
      { scenario: "You get a voice message that sounds exactly like your best friend, urgently asking you to send them some money.", ask: "What should you remember?", options: [{ label: "It really is them, a voice can't be copied" }, { label: "A voice can be cloned now, so check another way before you act", correct: true }, { label: "Send it, it sounds just like them" }] },
      { scenario: "You want a check that still works on a flawless fake.", ask: "Which one holds up?", options: [{ label: "Count the fingers in the image" }, { label: "It looks real to me" }, { label: "Do reliable sources report the same thing?", correct: true }] },
      { scenario: "A viral tip claims: 'You can always spot a fake because the shadows look wrong.'", ask: "Why is that a weak plan?", options: [{ label: "As fakes improve, tells like shadows get fixed, so it stops working", correct: true }, { label: "It's perfect, shadows never fool anyone" }, { label: "Fakes never include shadows at all" }] },
      { scenario: "A claim surprises you and you want the truth.", ask: "What is 'reading sideways'?", options: [{ label: "Read the suspicious page more slowly" }, { label: "Leave the page and see what the rest of the web says", correct: true }, { label: "Trust the page's own 'about' text" }] },
      { scenario: "A website you've never heard of posts a huge claim, and you want to read sideways to check it.", ask: "What do you actually do?", options: [{ label: "Read the site's own pages until you feel sure" }, { label: "Trust it because the page looks professional" }, { label: "Open new tabs and search the site and the claim elsewhere", correct: true }] },
      ...
      { scenario: "A post says a rival team's star player cheated, and it comes from a fan account of the other team.", ask: "What should you weigh first?", options: [{ label: "Fan accounts are always fair and neutral" }, { label: "Who benefits, the rival fans gain if you believe it", correct: true }, { label: "Believe it, it's about sport so it must be true" }] },
      { scenario: "A screenshot is going round but the original can't be found.", ask: "How should you treat it?", options: [{ label: "True, screenshots don't lie" }, { label: "Share it to ask if it's real" }, { label: "Unproven, no findable original means no proof", correct: true }] },
      { scenario: "A short, dramatic video clip is spreading, and it's clearly cut off at both ends.", ask: "What's the smart move?", options: [{ label: "Find the full, uncut original before you trust it", correct: true }, { label: "A short clip always tells the whole story" }, { label: "Re-share the clip so others can judge" }] },
      { scenario: "GHOSTWRITER pays bots and stirs up angry users so a fake travels faster than the truth.", ask: "Which stage of the play is that?", options: [{ label: "Manufacture, making the fake" }, { label: "Amplify, spreading it fast and wide", correct: true }, { label: "Profit, cashing in at the end" }] },
      { scenario: "A fake story ends up driving floods of clicks to ads and selling a 'miracle' gadget.", ask: "Which part of GHOSTWRITER's play is that?", options: [{ label: "Seed, dropping it where it catches" }, { label: "It has no real purpose" }, { label: "Profit, cashing in on the fake", correct: true }] },
      ...
      { scenario: "A post makes you want to smash the share button this second.", ask: "What's the circuit-breaker move?", options: [{ label: "Share fast, that strong urge means it's important" }, { label: "Treat that urge as a signal to pause and check first", correct: true }, { label: "Add 'not sure if true' and post it anyway" }] },
    ],
  },
