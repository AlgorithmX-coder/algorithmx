...
- **A.5 — AMPLITUDE OVERRIDE (owner-directed, 2026-07-13).** After repeated owner verdicts that the shipped restraint read as boring, the §3–§4 restraint mandates are superseded for implementation: Signal Room renders as a **neon holographic command deck** — full brand cyan `#00E5FF` edge-glow on panels/cards/bubbles, lit-circuitry hairlines (`#33506B`), backdrop at high amplitude (breathing color gas, glowing radar, luminous console chatter, full-room CRT scanlines), beat washes at ~44 alpha. The attention-budget *principle* survives inside play surfaces (one instruction strip, one CTA), but "neutrals ≥85%" and "glow is information only" no longer bind presentation. Do not regress toward "quiet" without a new owner ruling.
